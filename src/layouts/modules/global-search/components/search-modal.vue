...
const visible = defineModel('show', { required: true });
...
<template>
  ...
  >
    <NInputGroup>
      <NInput v-model:value="keyword" clearable :placeholder="$t('keyword_search')" @input="handleSearch">
        <template #prefix>
    ...
    <div class="mt-20px">
      <NEmpty v-if="resultOptions.length === 0" :description="$t('no_data')" />
