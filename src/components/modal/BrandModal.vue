  ...
  <ReusableSingleModal v-model:isVisible="isVisibleModel" :title="formTitle" width="480px" @save="handleSave" @close="handleClose">
    <n-form :model="localFormData" label-placement="top" class="grid grid-cols-1 gap-3 mt-2">
      <n-form-item :label="$t('code')" path="code" required>
        <n-input v-model:value="localFormData.code" :placeholder="$t('please_input')" clearable />
      </n-form-item>
      ...
      <n-form-item :label="$t('status')" path="statusId" v-if="isEdit">
        <n-select v-model:value="localFormData.statusId" :options="brandStatusOptions" :placeholder="$t('please_select')" filterable clearable />
      </n-form-item>
    </n-form>
  </ReusableSingleModal>
</template>
